...
$nums = Db::table('app_duocang')->count();
		...
		<div class="page-title-box">
			<div class="page-title-right">
				<ol class="breadcrumb m-0">
					...
					<li class="breadcrumb-item active">仓库</li>
				</ol>
			</div>
				...
				<form action="" method="post" name="form_log" id="form_log">
					<div class="table-responsive">
						<table class="table table-centered table-striped dt-responsive nowrap w-100">
							...
							<tbody>
								<?php
									$app = Db::table('app_duocang', 'as K')->field('K.*,A.name as appname')->JOIN('app', 'as A', 'K.appid=A.id');
									if ($so) {
								...
								?>
									<tr>
										...
										<td>
											<center><?php if ($rows['status_dcjm'] == 'n') : ?><span class="badge badge-danger">停用<?php else : ?><span class="badge badge-success">启动<?php endif; ?></span></center>
										</td>
										<td>
											<center>
										...
										<td>
											<center><a href="javascript:void(0);" onclick="modal_cut('edit',<?php echo $rows['id']; ?>,'<?php echo $rows['url']; ?>','<?php echo $rows['name']; ?>','<?php echo $rows['status']; ?>','<?php echo $rows['status_dcjm']; ?>','<?php echo $rows['appid']; ?>')" class="action-icon"> <i class="mdi mdi-border-color" data-toggle="modal" data-target="#modal"></i></a></center>
										</td>
									</tr>
								<?php } ?>
							</tbody>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
					<form class="pl-6 pr-6" method="post">
						<input class="form-control" type="number" id="dc_id" name="dc_id" value="" placeholder="配置ID" hidden>

						<div class="form-group">
							<label class="col-form-label">名称</label>
							<input class="form-control" type="text" id="dc_name" name="dc_name" placeholder="多仓上显示和搜索的名称" required>
						</div>
						<div class="form-group">
							<label>链接</label>
							<input class="form-control" type="text" id="dc_url" name="dc_url" placeholder="链接" required>
						</div>

						
						<div class="form-group">
							<label>绑定应用</label>
							<select class="form-control" name="dc_appid" id="dc_appid">
								<?php
						...
						<div class="eruyi-checkbox">
							...
							<input type="checkbox" id="status_dcjm" <?php if ($rows['status_dcjm'] == 'y') : ?>checked<?php endif; ?> data-switch="success" onchange="" />
							<label for="status_dcjm" data-on-label="开启" data-off-label="关闭"></label>
						</div>
...
<script>
	$('#add_submit').click(function() {
		...
		var status_dcjm = document.getElementById("status_dcjm").checked;
		if (status_dcjm) {
			status_dcjm = 'y';
		} else {
			status_dcjm = 'n';
		...
		let t = window.jQuery;
		var dc_url = $("input[name='dc_url']").val();
		var dc_name = $("input[name='dc_name']").val();
		var dc_appid = $("select[name='dc_appid']").val();
		...
		$.ajax({
			cache: false,
			type: "POST", //请求的方式
			url: "ajax.php?act=app_dc_add", //请求的文件名
			data: {
				url: dc_url,
				name: dc_name,
				status: status,
				status_dcjm: status_dcjm,
				appid:dc_appid
			},
			dataType: 'json',
	...
	$('#edit_submit').click(function() {
		...
		$.ajax({
			cache: false,
			type: "POST", //请求的方式
			url: "ajax.php?act=app_dc_edit", //请求的文件名
			data: {
				id: dc_id,
				url: dc_url,
				name: dc_name,
	...
	function modal_cut(type,id, url, name, status, status_dcjm,appid) {
		if (type == 'add') {
			...
			document.getElementById("status_dcjm").value = '';
			...
			document.getElementById("dc_url").value = url;
			document.getElementById("dc_name").value = name;
			document.getElementById("dc_appid").value = appid;
			if (status == 'y') {
			...
			if (status_dcjm == 'y') {
				document.getElementById("status_dcjm").checked = true;
			} else {
				document.getElementById("status_dcjm").checked = false;
			}
		}
	}
	...
	function delsubmit() {
		...
		$.ajax({
			cache: false,
			type: "POST", //请求的方式
			url: "ajax.php?act=app_dc_del", //请求的文件名
			data: {
				id: id_array
			},
			dataType: 'json',
