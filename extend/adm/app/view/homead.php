	...
	<div class="col-12">
		<div class="card">
			<div class="card-body">
				<div class="row mb-2">
					<div class="col-lg-8">
						<button type="button" onclick="modal_cut('add',0,0,0,0)" class="btn btn-danger mb-2 mr-2" data-toggle="modal" data-target="#modal"><i class="mdi mdi-briefcase-plus-outline mr-1"></i>添加轮播图片</button>
					</div>
					<div class="col-lg-4">
							...
							<thead>
								<tr>
									...
									<th style="width: 150px;">
										<center><span class="badge badge-light-lighten">地址</span></center>
									</th>
									<th style="width: 150px;">
							...
							<tbody>
								...
								?>
									<tr>
										...
										<td>
											<center><?php echo $rows['data']; ?></center>
										</td>
										<td>
										...
										<td>
											<center><a href="javascript:void(0);" onclick="modal_cut('edit',<?php echo $rows['id']; ?>,'<?php echo $rows['name']; ?>','<?php echo $rows['data']; ?>','<?php echo $rows['type']; ?>',<?php echo $rows['appid']; ?>,'<?php echo $rows['searchable']; ?>')" class="action-icon"> <i class="mdi mdi-border-color" data-toggle="modal" data-target="#modal"></i></a></center>
										</td>
									</tr>
								<?php } ?>
							</tbody>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title" id="modal_title">添加轮播图片</h4>
				...
				<?php if ($app_num > 0) : ?>
					<form class="pl-6 pr-6" method="post">
						...
						<div class="form-group">
							...
							<input class="form-control" type="text" id="homead_name" name="homead_name" placeholder="图片名称" required>
						</div>

						<div class="form-group">
							<label>图片地址</label>
							<textarea id="homead_data" name="homead_data" class="form-control" maxlength="255" rows="4" placeholder="图片地址"><?php echo $res['homead_data']; ?></textarea>
						</div>
						<div class="form-group">
							<label>类型</label>
							<select class="form-control" name="homead_type" id="homead_type" onchange="adType_change()">
...
<script>
	function adType_change() {
		if ($('#homead_type').val() == 'img') {
	...
	$('#add_submit').click(function() {
		...
		var homead_type = $("select[name='homead_type']").val();
	...
	function modal_cut(type, id, name, data, type1, appid, searchable) {
		if (type == 'add') {
			document.getElementById("modal_title").innerHTML = "添加轮播图配置";
			...
			document.getElementById("homead_type").value = 'img';
		} else {
			document.getElementById("modal_title").innerHTML = "编辑轮播图配置";
			...
			document.getElementById("homead_type").value = type1;
