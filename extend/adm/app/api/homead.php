...
if ($act == 'add') { //添加配置
	...
	$type = isset($_POST['type']) ? purge($_POST['type']) : 'img';
	...
	$add_res = Db::table('app_homead')->add(['name' => $name, 'data' => $data, 'type' => $type, 'appid' => $appid, 'searchable' => $searchable]);
	//die($add_res); 
	if ($add_res) {
...
if ($act == 'edit') { //编辑配置
	...
	$update['type'] = isset($_POST['type']) ? purge($_POST['type']) : 'img';
